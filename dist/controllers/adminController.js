...
const timeDate = (value) => new Date(`1970-01-01T${value.length === 5 ? `${value}:00` : value}Z`);
const timeString = (value, fallback = '09:00:00') => value ? value.toISOString().slice(11, 19) : fallback;
...
exports.listCompanyRoles = listCompanyRoles;
...
exports.createCompanyRole = createCompanyRole;
...
const generateQr = async (req, res) => {
    try {
        ...
        const resp = await (0, qrService_1.qrResponse)(qrToken);
        res.json(resp);
    }
    catch (error) {
...
const getLatestQrToken = async (req, res) => {
    try {
        ...
        const resp = await (0, qrService_1.qrResponse)(latest);
        res.json(resp);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
exports.getLatestQrToken = getLatestQrToken;
...
exports.saveAttendanceSettings = saveAttendanceSettings;
...
const analytics = async (req, res) => {
    try {
        res.json(await (0, analyticsService_1.monthAnalytics)(req.query.month));
    }
    catch (error) {
        res.status(400).json({ error: error.message });
    }
...
exports.saveActiveOfficeLocation = saveActiveOfficeLocation;
...
const employeeLeaveBalances = async (req, res) => {
    try {
        const employeeId = Number(req.params.id);
...
exports.assignEmployeeOfficeLocation = assignEmployeeOfficeLocation;
...
exports.updateEmployeeUsername = updateEmployeeUsername;
...
exports.bulkResetPasswords = bulkResetPasswords;
const bulkEditEmployees = async (req, res) => {
    try {
        const ids = (req.body.employeeIds || []).map(Number).filter(Boolean);
        if (!ids.length)
            throw new Error('Select at least one employee');
        const data = {};
        if ('officeLocationId' in req.body)
            data.assignedOfficeLocationId = parseOptionalNumber(req.body.officeLocationId);
        if ('departmentId' in req.body)
            data.departmentId = parseOptionalNumber(req.body.departmentId);
...
exports.bulkEditEmployees = bulkEditEmployees;
...
exports.employeesCsv = employeesCsv;
...
exports.setEmployeeEnabled = setEmployeeEnabled;
...
const uploadCompanyPhoto = async (req, res) => {
    try {
        const file = req.file;
        if (!file)
            throw new Error('Photo file is required');
        const upload = await (0, cloudinaryService_1.uploadGroupPhoto)(file.buffer, 'company-group-photo');
...
const saveProductionPolicy = async (req, res) => { res.json({ ok: true, versionName: req.body.versionName || 'Production policy', savedAt: new Date().toISOString() }); };
exports.saveProductionPolicy = saveProductionPolicy;
const approveProductionDevice = async (req, res) => { res.json({ ok: true, id: Number(req.params.id), approved: Boolean(req.body.approved) }); };
exports.approveProductionDevice = approveProductionDevice;
const statutoryReport = async (req, res) => {
    try {
        const month = String(req.query.month || new Date().toISOString().slice(0, 7));
        const start = new Date(`${month}-01T00:00:00Z`);
        const end = new Date(start);
