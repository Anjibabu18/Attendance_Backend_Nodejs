...
`;
const send = async (subject, body, to) => {
    if (!mailEnabled || to.length === 0)
        return;
    try {
...
const notifyHr = async (subject, body) => {
    const hrs = await getHrRecipients();
    await send(subject, body, hrs);
};
exports.notifyHr = notifyHr;
const notifyUser = async (to, subject, body) => {
